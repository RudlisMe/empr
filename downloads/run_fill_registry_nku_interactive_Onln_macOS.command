...
# Интерактивный запуск заполнения реестра НКУ для macOS
# Можно переносить в любую папку вместе с:
# - fill_registry_nku_Onln_v9.py
# - google_service_account.json для онлайн-режима
# - Excel-файлом .xlsx для локального режима
#
...
SCRIPT_NAME="fill_registry_nku_Onln_v9.py"

if [ ! -f "$SCRIPT_NAME" ]; then
    echo "ОШИБКА: $SCRIPT_NAME не найден рядом с этим файлом."
...
while true; do
    ...
    case "$MODE" in
        ...
        2)
            ...
            echo "Если рядом несколько .xlsx, он попросит оставить один или запустить с явным путём."
            echo ""
            "$VENV_PY" "$SCRIPT_NAME"
            break
            ;;
